...
fn test_mimc_groth16() {
    ...
    for _ in 0..SAMPLES {
        ...
        let start = Instant::now();
        {
            ...
            let mut proof_vec = vec![];
            proof.serialize_uncompressed(&mut proof_vec).unwrap();
            let mut inputs_vec = vec![];
            vec![image].serialize_uncompressed(&mut inputs_vec).unwrap();
            println!("image: {}", image);
            let mut vk_vec = vec![];
            vk.serialize_uncompressed(&mut vk_vec).unwrap();
            println!("[vk, proof, inputs]: {{\"{}\", \"{}\", \"{}\"}}", BASE64_STANDARD.encode(&vk_vec), BASE64_STANDARD.encode(&proof_vec), BASE64_STANDARD.encode(&inputs_vec));
        }
